                    ...
                    <AccordionContent>
                      <ul className="space-y-2">
                        {subtopic.lessonContent.map((content, contentIndex) => (
                          <li key={contentIndex}>
                            <Link
                              href={`/teas-test/${subject.name.toLowerCase()}/${topic.title.toLowerCase().replace(/\s+/g, '-')}/${content.type}`}
                              className={cn(
                                "flex items-center space-x-2 text-sm hover:underline",
                                pathname === `/teas-test/${subject.name.toLowerCase()}/${topic.title.toLowerCase().replace(/\s+/g, '-')}/${content.type}`
