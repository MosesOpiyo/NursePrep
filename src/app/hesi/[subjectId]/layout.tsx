...
export default async function SideNav({
  ...
  return (
    <section className="hesi-layout flex flex-col sidebar">
        ...
        <div className="sidenav-container overflow-y-auto w-1/4 border-x-2 border-solid p-4 flex-col gap-8 hidden md:flex">
          ...
          {subject.content.map((topic) => (
            <div key={topic.id}>
              ...
              <Link
                href={`/hesi/${params.subjectId}/topics/${topic.id}/${topic.id}`}
                className="flex items-center gap-2"
              >
                &nbsp;&nbsp;<FaBook />{topic.subtopic}
              </Link>

              <Link href={`/hesi/${params.subjectId}/quizes/${topic.id}/${topic.id}`} className="flex items-center gap-2">
